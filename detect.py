...
def run(
    ...
    # print(summary(model, input_size = imgsz))

    # Dataloader    bs = 1  # batch_size
    if webcam:
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            # cv2.imwrite(str(Path(save_path+str(frame)+'.png').with_suffix('.png')),im0)
            # str(Path(save_path).with_suffix('.mp4'))
...
#         else:
#             p = Path(p)
#         txt_path = str('/local/home/tongxi/testbatchdetect/blah11/labels') + '/'+ p.name[:-4] + ('' if dataset.mode == 'image' else f'_{frame}')+'.txt'  # im.txt
#         if not os.path.exists(txt_path): continue
#         with open(txt_path, 'r') as rfile:
#             pred = np.asarray([[each.split(' ') for each in rfile.read().split('\n') if len(each)>0]])
#         # with dt[0]:
#         #     im = torch.from_numpy(im).to(model.device)
#         #     im = im.half() if model.fp16 else im.float()  # uint8 to fp16/32
#         #     im /= 255  # 0 - 255 to 0.0 - 1.0
...
#                     s += f"{n} {names[int(c)]}{'s' * (n > 1)}, "  # add to string
#                 # Write results
#                 for cls, *xywh, conf in det:
#                     xywh = [float(each) for each in xywh]
#                     conf = float(conf)
#                     # if save_txt:  # Write to file
#                         # xyxy = xyxy2xywh(torch.tensor(xywh).view(1, 4)).view(-1).tolist()
#                         # xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()  # normalized xywh
#                         # line = (cls, *xywh, conf) if save_conf else (cls, *xywh)  # label format
#                         # with open(f'{txt_path}.txt', 'a') as f:
#                         #     f.write(('%g ' * len(line)).rstrip() % line + '\n')
#                     xyxy = xyxy2xywh(torch.FloatTensor(xywh).view(1, 4)*gn).view(-1).tolist()
