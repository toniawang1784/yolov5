...
class LoadDicom:
    def __init__(self,path,img_size=640,stride=32,auto=True, transforms=None,vid_stride=1):
        ...
        # for lumify studies temp #
        # verticalsweep = ['RR', 'R', 'M', 'LL', 'L', 'R0', 'R1', 'L0', 'L1']
        # horizontalsweep = ['C1', 'C2', 'C3', 'C4', 'C5']
        # # print([x for x in files])
        # dicom_sweep = [x.split('_')[0].split('/')[-1] for x in files]
        # sweep_wanted = [each in verticalsweep+horizontalsweep for each in dicom_sweep]
        # dicoms = [each for i,each in enumerate(files) if sweep_wanted[i] ]
        dicoms=[x for x in files] # if x.split('.')[-1].lower() in DICOM_FORMATS]
        # end of lumify studies temp #
        print(len(dicoms))
        self.nf=len(dicoms)
        self.img_size = img_size
        self.mode = 'video'
...
def create_classification_dataloader(path,
    ...
    return InfiniteDataLoader(dataset,
                              ...
                              pin_memory=PIN_MEMORY,
                              worker_init_fn=seed_worker,
                              generator=generator)  # or DataLoader(persistent_workers=True)# # YOLOv5 🚀 by Ultralytics, GPL-3.0 license
# """
# Dataloaders and dataset utils
# """
...
#                 print(p)
#                 raise FileNotFoundError(f'{p} does not exist')
#         possibleVerticalSweeps_sorted    = ['RR','R2','R1','R0','R','NR','NM','M','NL','L','L0','L1','L2', 'LL'] # Add L3, R3? Also in YOLO results? # ,'NR','NM','NL' are perhaps typos in FAMLI daa since it is not in their protocol
#         possibleLateralSweeps_sorted     = ['C0','C1','C2','C3','C4','C5','C6', 'NC1', 'NC2', 'NC3', 'NC4', 'NC5']
#         dicoms=[x for x in files if os.path.getsize(os.path.join(path, x))>1000000] # if x.split('.')[-1].lower() in DICOM_FORMATS]
#         studyname = path.rsplit('/', 2)[-1]
#         txt_path = str('/local/data/FAMLI/yolo_inference_csvs/{}.csv'.format(studyname))  # im.txt
#         df_study = pd.read_csv(txt_path).to_numpy()
#         # sops = np.unique(df_study[:, 10])
#         # sweeps = df_study[df_study[:, 10]==sop, 1]
#         sops = []
#         for dicom in dicoms:
#             sop = dicom.rsplit('/')[-1]
#             sweep = df_study[df_study[:, 10]==sop, 1]
#             sweep = np.unique(sweep).tolist()
#             assert len(sweep)==1
#             sweep = sweep[0]
...
# class LoadImages:
#     # YOLOv5 image/video dataloader, i.e. `python detect.py --source image.jpg/vid.mp4`
#     def __init__(self, path, batch_size = None, img_size=640, stride=32, auto=True, transforms=None, vid_stride=1):
#         if isinstance(path, str) and Path(path).suffix == ".txt":  # *.txt file with img/vid/dir on each line
#             path = Path(path).read_text().rsplit()
#         files = []
#         for p in sorted(path) if isinstance(path, (list, tuple)) else [path]:
