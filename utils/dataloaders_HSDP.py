...
FAMLI_SHARED_DRIVE_FILENAMES_PATH = '/home/pj-024018/data/FAMLI1/GA_NEJME_ultrasound_instances.csv'
...
class LoadDicom:
    def __init__(self,path,img_size=640,stride=32,auto=True, transforms=None,vid_stride=1):
        ...
        dicoms_sweep = []
        famli_csv = pd.read_csv(FAMLI_SHARED_DRIVE_FILENAMES_PATH)
        # examname = path.rsplit('/', 1)[-1]
        examname = 'FAM-202-1375-1'
        for x in dicoms:
            sop = x.rsplit('/', 1)[-1]
            findrow = famli_csv.query("study_id_with_acquisition_source==@examname and instance==@sop")
            sweepname = findrow['tag'].iloc[0].replace('N', '')
            if sweepname in verticalsweep+horizontalsweep: dicoms_sweep.append(x)
        dicoms = dicoms_sweep
        dicom_sizes=[os.path.getsize(os.path.join(path, x)) for x in dicoms]
        print('Number of dicoms in current study: ', len(dicoms))
        self.nf=len(dicoms)
        self.img_size = img_size
        self.mode = 'video'
        self.auto = auto
