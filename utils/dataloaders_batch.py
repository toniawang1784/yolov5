...
class LoadImages:
    ...
    def __init__(self, path, batch_size = 4, img_size=640, stride=32, auto=True, transforms=None, vid_stride=1):
        if isinstance(path, str) and Path(path).suffix == ".txt":  # *.txt file with img/vid/dir on each line
            path = Path(path).read_text().rsplit()
        files = []
        for p in sorted(path) if isinstance(path, (list, tuple)) else [path]:
    ...
    def __next__(self):
        if self.count >= self.nf:
            raise StopIteration
        # path = self.files[self.count] 
        if self.count+self.bs <= self.nf:
            path = self.files[self.count:self.count+self.bs]
        else: 
            path = self.files[self.count:]
        if self.video_flag[self.count]:
            # Read video
            ...
            for eachPath in path:
                im_TMP = cv2.imread(eachPath)
                assert im_TMP is not None, f'Image Not Found {eachPath}'
                im0.append(im_TMP)
                s = f'image {self.count}/{self.nf} {eachPath}: '

        if self.transforms:
            im = [self.transforms(eachimg) for eachimg in im0]  # transforms
            im = np.asarray(im)
        else:
            im = [letterbox(eachimg, self.img_size, stride=self.stride, auto=self.auto)[0] for eachimg in im0]  # padded resize
            im = [eachimg.transpose((2, 0, 1))[::-1] for eachimg in im]  # HWC to CHW, BGR to RGB
            im = [np.ascontiguousarray(eachimg) for eachimg in im] # contiguous
            # if im.ndim == 3:
            #     im = im[:, :, :]
            #     #im = np.broadcast_to(im,(1,im.shape[0],im.shape[1],im.shape[2]))
            #print(im.shape)
...
class LoadDicomsAndLabels(Dataset):
    # YOLOv5 train_loader/val_loader, loads images and labels for training and validation
    cache_version = 0.6  # dataset labels *.cache version
    rand_interp_methods = [cv2.INTER_NEAREST, cv2.INTER_LINEAR, cv2.INTER_CUBIC, cv2.INTER_AREA, cv2.INTER_LANCZOS4]
