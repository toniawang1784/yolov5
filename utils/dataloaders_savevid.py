...
class LoadDicom:
    def __init__(self,path,bs=16,img_size=640,stride=32,auto=True, transforms=None,vid_stride=1,device=None):
    ...
    def __next__(self):
        ...
        if self.transforms:
            ...
            # im=np.asarray([self.extract_cone(np.expand_dims(eachimg,0))[0] for eachimg in im0])
            # im = np.squeeze(im, axis = 0)
            # im = torch.from_numpy(im).to(self.device)
            # im=np.broadcast_to(im,(im.shape[0],im.shape[1],im.shape[2],3))
            im = [letterbox(eachim, self.img_size, stride=self.stride, auto=self.auto)[0] for eachim in im0]
            im=np.transpose(np.asarray(im),(0,3,1,2))
    ...
    def _new_sweep(self, path):
        ...
        # self.arr = torch.as_tensor(self.arr).to(self.device)
